...
// std::string TorrentFileParser::extractString(const BencodedValue& dict, const std::string& key) {
//     if (!std::holds_alternative<std::map<std::string, BencodedValue>>(dict)) {
//         throw std::runtime_error("Expected a dictionary");
//     }

//     auto& map = std::get<std::map<std::string, BencodedValue>>(dict);
...
//     if (!std::holds_alternative<int64_t>(map.at(key))) {
//         throw std::runtime_error("Expected an integer for key: " + key);
//     }

//     return std::get<int64_t>(map.at(key));
// }
...
//     auto& map = std::get<std::map<std::string, BencodedValue>>(dict);
//     if (map.find("pieces") == map.end()) {
//         throw std::runtime_error("Missing 'pieces' key in info dictionary");
//     }

//     if (!std::holds_alternative<std::string>(map.at("pieces"))) {
//         throw std::runtime_error("Expected a string for 'pieces'");
//     }

//     std::string piecesStr = std::get<std::string>(map.at("pieces"));
//     std::vector<std::string> pieces;
...
//     auto& map = std::get<std::map<std::string, BencodedValue>>(dict);
//     if (map.find("files") == map.end()) {
//         throw std::runtime_error("Missing 'files' key in info dictionary");
//     }

//     if (!std::holds_alternative<std::vector<BencodedValue>>(map.at("files"))) {
//         throw std::runtime_error("Expected a list for 'files'");
//     }

//     auto& filesList = std::get<std::vector<BencodedValue>>(map.at("files"));
//     std::vector<std::pair<std::string, int64_t>> files;

//     for (const auto& fileDict : filesList) {
//         if (!std::holds_alternative<std::map<std::string, BencodedValue>>(fileDict)) {
//             throw std::runtime_error("Expected a dictionary for file entry");
//         }

//         auto& fileMap = std::get<std::map<std::string, BencodedValue>>(fileDict);
//         int64_t length = extractInt(fileMap, "length");
...
//         if (!std::holds_alternative<std::vector<BencodedValue>>(fileMap.at("path"))) {
//             throw std::runtime_error("Expected a list for 'path'");
//         }

//         auto& pathList = std::get<std::vector<BencodedValue>>(fileMap.at("path"));
//         std::string path;

//         for (const auto& pathComponent : pathList) {
//             if (!std::holds_alternative<std::string>(pathComponent)) {
//                 throw std::runtime_error("Expected a string for path component");
//             }

//             path += std::get<std::string>(pathComponent) + "/";
//         }
