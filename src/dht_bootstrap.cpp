...
namespace DHT {
    ...
    //         return nodes;
    //     }
    //     // In send_find_node_request, after creating the socket:
    //     #ifdef _WIN32
    ...
    //     // oss << "FIND_NODE " << node_id_to_hex(target_id);
        
    //     // std::string request = oss.str();

    //     // Replace the request creation code with bencoded formatting
    //     std::string transaction_id = "aa"; // Simple transaction ID
    //     std::string query_type = "find_node";

    //     // Bencoded dictionary
    //     std::ostringstream oss;
    //     oss << "d"
    //         << "1:a"      // Inner dictionary under key "a"
    //         << "d"
    //         << "2:id"     // Your node ID (20 bytes)
    //         << "20:" << std::string(reinterpret_cast<const char*>(my_node_id_.data()), 20)
    //         << "6:target" // Target node ID (20 bytes)
    //         << "20:" << std::string(reinterpret_cast<const char*>(target_id.data()), 20)
    //         << "e"        // End of inner dictionary
    //         << "1:q"      // Query type
    //         << "9:" << query_type
    //         << "1:t"      // Transaction ID
    //         << "2:" << transaction_id
    //         << "1:y"      // Message type (q for query)
    //         << "1:q"
    //         << "e";
    ...
    //         // Parse nodes in compact format
    //         const char* compact_nodes = buffer;
    //         size_t num_nodes = bytes_received / 26;
    
    //         for (size_t i = 0; i < num_nodes; ++i) {
    //             const char* node_data = compact_nodes + i * 26;
    
    //             Node node;
    //             // Copy NodeID
    //             std::memcpy(node.id.data(), node_data, 20);
    
    //             // Extract IPv4
    //             struct in_addr ip_addr;
    //             std::memcpy(&ip_addr.s_addr, node_data + 20, 4);
    ...
    //             std::memcpy(&port_network, node_data + 24, 2);
    //             node.port = ntohs(port_network);
    
    //             // Log the parsed node
    //             std::cout << "Parsed Node: " << node.ip << ":" << node.port 
    //                       << " (ID: " << node_id_to_hex(node.id) << ")" << std::endl;
